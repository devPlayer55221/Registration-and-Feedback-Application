...
public:
	...
	Timer(const function<void(void)>& f, const unsigned long& i, const long repeat = Timer::Infinite) : funct(f), interval(std::chrono::milliseconds(i)), CallNumber(repeat) {};
	...
	void SetFunction(const function<void(void)>& f) // sets the func to be executed
	{
		funct = f;
	}
